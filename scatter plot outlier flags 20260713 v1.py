...
def calculate_outlier_flags(group, config):
    """
    Calculate outlier flags for a peer group using robust Z-score, IQR,
    ...
    Returns:
        pandas.DataFrame: Peer group with outlier flag columns added.
    """
    group = group.copy()

    group["robust_z_score"] = calculate_robust_z_scores(group["Procedure Price"])
    group["is_robust_z_outlier"] = (
        group["robust_z_score"].abs() >= config.robust_z_threshold
    )

    q1 = group["Procedure Price"].quantile(0.25)
    q3 = group["Procedure Price"].quantile(0.75)
    iqr = q3 - q1

    if pd.notna(iqr):
        lower_iqr = q1 - 1.5 * iqr
        upper_iqr = q3 + 1.5 * iqr
        group["is_iqr_outlier"] = (
            (group["Procedure Price"] < lower_iqr)
            | (group["Procedure Price"] > upper_iqr)
        )
    else:
        group["is_iqr_outlier"] = False

    p_low = group["Procedure Price"].quantile(config.lower_percentile / 100)
    p_high = group["Procedure Price"].quantile(config.upper_percentile / 100)

    if pd.notna(p_low) and pd.notna(p_high):
        group["is_percentile_extreme"] = (
            (group["Procedure Price"] <= p_low)
            | (group["Procedure Price"] >= p_high)
        )
    else:
        group["is_percentile_extreme"] = False
...
def get_flagged_rows(group):
    """
    Return rows flagged by any outlier detection method.
...
def build_flag_text(row):
    ...
    if row["is_robust_z_outlier"]:
        flag_types.append("Robust Z-score")
    if row["is_iqr_outlier"]:
        flag_types.append("IQR")
    if row["is_percentile_extreme"]:
        flag_types.append("Percentile")
...
def plot_peer_group(ax, group, bin_label, config):
    """
    Plot the base scatter points and median line for one peer group.
    ...
    median_price = group["Procedure Price"].median()
    xmin = group["Procedure Units"].min()
    xmax = group["Procedure Units"].max()
...
def format_plot(ax):
    ...
    ax.set_ylabel("Average Procedure Price per Customer (USD)", fontsize=15)
    ax.set_xscale("log")
    ax.set_title(
        "Customer Scatter Plot by Annual Volume Bin with Outlier Flags",
        fontsize=15,
    )
...
def add_legends(ax, numbered_customers):
    ...
    customer_handles = [
        Line2D(
            ...
            markersize=10,
            label=f"{num}: {cust} ({peer}) - {flags}",
        )
        for num, cust, peer, flags in numbered_customers
    ]
...
def create_scatter_plot(df, config):
    """
    Create the scatter plot with peer groups, outlier highlighting,
    and customer annotations.
    ...
    for bin_label, group in df.groupby("Peer Group", observed=True):
        ...
        group = calculate_outlier_flags(group, config)
