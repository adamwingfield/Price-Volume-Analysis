...
def calculate_expected_price_and_residuals(group):
    """
    Model procedure price as a linear function of procedure units within a
    peer group and calculate expected price and residuals.
    ...
    Returns:
        pandas.DataFrame: Group with expected price and residual columns added.
    """
    group = group.copy()

    if len(group) < 2 or group["Procedure Units"].nunique() < 2:
        expected_price = group["Procedure Price"].median()
        group["Expected Price"] = expected_price
    ...
    group["Expected Price"] = intercept + slope * group["Procedure Units"]
    group["Residual"] = group["Procedure Price"] - group["Expected Price"]
...
def calculate_outlier_flags(group, config):
    """
    Calculate outlier flags based on residuals using robust Z-score, IQR,
    ...
    group["Residual Robust Z-Score"] = calculate_robust_z_scores(group["Residual"])
    ...
    if pd.notna(iqr):
        ...
        group["Is Residual IQR Outlier"] = (
            (group["Residual"] < lower_iqr) | (group["Residual"] > upper_iqr)
        )
    else:
        group["Is Residual IQR Outlier"] = False

    p_low = group["Residual"].quantile(config.lower_percentile / 100)
    p_high = group["Residual"].quantile(config.upper_percentile / 100)

    if pd.notna(p_low) and pd.notna(p_high):
        group["Is Residual Percentile Extreme"] = (
            (group["Residual"] <= p_low) | (group["Residual"] >= p_high)
        )
    else:
...
def analyze_residuals(df, config):
    """
    Apply residual modeling and outlier detection to each peer group.
...
def build_flag_text(row):
    ...
    if row["Is Residual Robust Z Outlier"]:
        flag_types.append("Residual Robust Z-score")
    if row["Is Residual IQR Outlier"]:
        flag_types.append("Residual IQR")
    if row["Is Residual Percentile Extreme"]:
        flag_types.append("Residual Percentile")
...
def format_plot(ax):
    ...
    ax.set_xscale("log")
    ax.set_title(
        "Customer Price vs. Volume with Residual-Based Outlier Flags",
        fontsize=15,
    )
...
def add_legends(ax, numbered_customers):
    ...
    customer_handles = [
        ...
        for num, cust, peer, flags, residual in numbered_customers
    ]
...
def export_results_table(export_df):
    ...
    file_path = filedialog.asksaveasfilename(
        ...
        initialfile="customer_residual_analysis.xlsx",
    )
...
def main():
    ...
    model expected price, calculate residuals, create the plot,
    and export the results table.
    """
    config = CONFIG
